...
def orbTargets():
    try:
        ...
        instruments = get_instruments_filter_by_status(time_helper.get_current_date(), DB_ORDER_STATUS['INIT'])
        if not instruments:
...
def placeEntryOrders():
    try:
        ...
        instruments = get_instruments_filter_by_status(time_helper.get_current_date(), DB_ORDER_STATUS['CREATED'])
        if not instruments:
...
def checkEntryOrders():
    try:
        instruments = get_instruments_filter_by_status(time_helper.get_current_date(), DB_ORDER_STATUS['ENTRY_PLACED'])
        if not instruments:
...
def placeExitOrder():
    try:
        instruments = get_instruments_filter_by_symbols(time_helper.get_current_date(), active_position_symbols())
        if not instruments:
...
def checkExitOrder():
    try:
        instruments = get_instruments_filter_by_status(time_helper.get_current_date(),
                                                       DB_ORDER_STATUS['TARGETS_PLACED'])
        if not instruments:
...
def get_instruments_filter_by_status(date, status):
...
def get_instruments_filter_by_symbols(date, symbols):
    trade_items = trading_table.query(
        KeyConditionExpression=Key("pk").eq(date)
    )
