...
def add_text(background, text, logo_text):
...
def image_editor(background, background_name, text, logo_text):
    tmp_dir = '/tmp'
...
def make_image(content, logo_text=constants.LOGO_TEXT):
    image = get_images(content['Category'])
    return image_editor(image, "{}.jpg".format(str(round(time.time() * 1000))), content, logo_text)
